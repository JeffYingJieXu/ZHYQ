...
static char kWhenLongTappedBlockKey;
...
- (void)whenLongTapped:(JMWhenTappedBlock)block {
    UILongPressGestureRecognizer *longPressReger = [[UILongPressGestureRecognizer alloc]initWithTarget:self action:@selector(viewWasLongTapped:)];
    longPressReger.minimumPressDuration = 1;
    [self addGestureRecognizer:longPressReger];
    
    [self setBlock:block forKey:&kWhenLongTappedBlockKey];
}
...
- (void)viewWasLongTapped:(UILongPressGestureRecognizer *)longPressRecognizer{
    if (longPressRecognizer.state != UIGestureRecognizerStateBegan) return ; // except multiple pressed it !
    [self runBlockForKey:&kWhenLongTappedBlockKey];
}
